...
def visualize_option_price_vs_spot(option_class, label, spot_range, **kwargs):
    ...
    plt.title(f"{label}: Price vs Spot")
    ...
    for _ in range(M):
        ...
        if barrier_hit:
            if prices[-1] > K:
                ...
                knocked_out_paths.append(prices)
        else:
            other_paths.append(prices)

    # Plotting
    plt.figure(figsize=(10, 6))
    for path in knocked_out_paths:
        plt.plot(time_grid, path, color="red", alpha=0.3)
    for path in contributing_paths:
        plt.plot(time_grid, path, color="green", alpha=0.3)
    for path in other_paths:
        plt.plot(time_grid, path, color="gray", alpha=0.2)

    plt.axhline(B, color="blue", linestyle="--", label=f"Barrier Level (B={B})")
    plt.title(f"Up-and-In Barrier Call Option Paths\n($S_0$={S0}, K={K}, B={B})")
    plt.xlabel("Time (Years)")
    plt.ylabel("Stock Price")
    plt.legend(["Knocked Out Paths", "Contributing Paths", "Other Paths", f"Barrier Level (B={B})"])
    plt.grid(True)
    plt.tight_layout()
    plt.show()
